...
输出：`technical-design.md`，必要时产生 `artifacts/ddl/*.sql`、`research/*.md` 或项目长期记忆更新。

## 流程

1. 运行 `task status <task> --json`，确认任务处于 `technical_design` 且 PRD checkpoint 有效。完成：revision、仓库范围和全部 AC 已知。
2. 从 `CONTEXT.md` 关键决策索引中筛选当前仓库、模块和主题命中的 ADR，再围绕明确设计问题扫描实际入口、调用链、数据模型、复用点和边界。关键代码证据记录为 `repo_id / path / symbol / line`；实际采用的长期记忆证据用 inline code 记录仓库根相对路径。完成：每个关键设计判断都有代码、ADR 或项目文档证据，且没有静默违反已有决定，checkpoint 可哈希实际记忆依赖。
3. 完成固定影响扫描，逐项写出设计或“不适用”理由；区分当前事实与目标事实，差异写成待实现或迁移项。对证据无法确定且必须由用户选择的技术取舍调用 `grilling`，在当前请求中明确写出访谈主题、已知事实、待决范围和以全部 AC 获得一致设计落点为目标的完成标准。形成新的专业术语或难以逆转、脱离背景会令人意外且存在真实取舍的决定时调用 `sw-domain-modeling`。完成：全部 AC 有设计落点，跨章节字段和状态一致，长期记忆已同步或决定仅保留在任务中。
...
复杂调查可暂存 `research/`；只保留方案引用的证据。验证后的项目级事实提升到 `project/index.md`，仓库事实提升到 `project/repositories/<repo-id>.md`，不创建无法从入口发现的知识文件。
