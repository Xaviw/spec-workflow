...
4. 发现需求、设计、spec 或规范与事实冲突时，停止相关代码，更新受影响文档与 `decisions.md`，必要时重新确认。每个增量完成后立即同步 Slice、已失真任务文档和已验证的 `project/` 长期事实。完成：文档与实现一致；revision 冲突已通过重读解决。

## 收尾

所有 Slice 完成后运行 `task validate <task>` 并调用 `sw-code-review`。完成：校验通过、阻塞发现已解决或按规则记录，复审无阻塞。用户确认实施结果后，以最新 revision 进入 `verification` 并调用 `sw-verify`。
