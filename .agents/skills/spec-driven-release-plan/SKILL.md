...
3. 来源指纹由任务状态、verification、change items 和引用 commit 计算。任一来源变化都使已确认方案失效并回到 draft。
4. 用户明确确认完整方案后，运行 `iteration confirm-release-plan <iteration-id> --confirmed`。占位内容会阻止确认；确认同时记录方案文件哈希。
5. 提交方案与 `iteration.json`。
6. 实际发布完成后，只有用户明确确认才运行 `iteration done <iteration-id> --confirmed`。该动作只更新文档状态，不执行 push、部署或 DDL。
