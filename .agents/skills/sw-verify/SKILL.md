...
输出：当前 `verification.md`、阻塞项、残余风险，以及包含验收结论和完整提交计划的一次交付确认或确有必要的业务自测项。
...
3. diff 稳定后，按 `spec.md` 已去重的验证图执行一次最终矩阵；执行前再次展开 pre/post hook，若一项确定包含另一项，只保留覆盖更完整的命令。逐条形成 AC 证据，测试通过不能替代业务证据；认证、授权、租户、身份和数据隔离 AC 必须有动态负向证据。UI AC 使用 `ui-recreate` 只读验证模式；没有可复现基准时标为 `unverified`。完成：每个命令只执行一次，每个 AC 都有符合状态要求的证据或明确的人工判断、豁免、失败或未验证原因。
...
6. 到此停止，不提交、不进入 done。若证据完整，生成包含依赖顺序、各目标仓库提交、`task phase <task> done --confirmed`、工作流文档提交及每个完整 message 的精确计划，与验收摘要放在同一个确认问题中；该次授权同时接受 `verification.md` 并允许执行整个交付计划。若有人工判断项，只列对应 AC、操作和通过标准，完成后再展示同一份交付确认。完成：只提出一次可判定的交付授权，并在获得授权前保持 `verification`。

## 验收记录

全部 PRD AC 使用一张紧凑表记录状态与证据，命令结果只在完整验证矩阵记录一次，AC 行引用对应命令或证据位置，不重复抄写结果：

```markdown
| AC | 状态 | 证据 |
| --- | --- | --- |
| AC-001 | pass | <原生命令或仓库根相对路径及结果摘要> |
```
