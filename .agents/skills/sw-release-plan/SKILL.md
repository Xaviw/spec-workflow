...
1. 确认全部任务为 done/cancelled 后，先请求本 Skill 生成只读预览；预览成功时同时展示“A. 收口迭代、写入该方案并合并提交”和“B. 跳过方案并收口迭代”，附两种选择的完整 workflow commit message。预览失败时只提供 B。
2. 用户选择后由调用方运行 `iteration close <iteration> --confirmed`。选择 A 时把已审阅正文和授权交回本 Skill，只补实际 `ended_at` 标记并落盘；确认方案为 `fresh` 后，将 `iteration.json` 与 `release-plan.md` 合并为一次提交。选择 B 时不写方案，只提交迭代状态。
3. 不重复确认。选择 A 后方案写入失败时仍提交已授权的收口状态，报告失败且不回退 iteration；不追踪外部实际发布状态。
